...
export default function App() {
  ...
  // Removed active user mock property auto-assignment as per user request to start with an empty listings view (unless user creates properties)
  ...
  const handleAddProperty = async (newProperty: Property) => {
    ...
    if (!isFirebasePlaceholder) {
      try {
        ...
        handleFirestoreError(error, OperationType.CREATE, `properties/${newProperty.id}`);
        return;
      }
    }
  ...
  const myListings = useMemo(() => {
    if (!user) return [];
    const owned = properties.filter(p => p.ownerId === user.email || p.ownerId === user.id);
      ...
      <main className="flex-1 pb-20">
        ...
        <div className="bg-white py-4 md:py-6 mb-4 md:mb-6 border-b border-slate-100 overflow-hidden">
          ...
          >
                  ...
                  <div className="flex flex-wrap gap-4">
                    ...
                    <button 
                      onClick={() => {
                        ...
                        addToast('Menampilkan seluruh properti dengan promo spesial!', 'info');
                      }}
                      className="bg-white/5 backdrop-blur-xl text-white border border-white/10 px-6 sm:px-8 md:px-12 py-2.5 sm:py-3 md:py-4 rounded-full font-bold w-fit hover:bg-white/10 transition-all text-[9px] sm:text-[10px] md:text-xs uppercase tracking-[0.2em] active:scale-95 cursor-pointer"
                    >
                      Lihat Promo
                    </button>
                  </div>
            ...
            <div className="flex sm:flex-col gap-4 snap-x snap-mandatory min-w-[170vw] sm:min-w-0 sm:h-[380px] md:h-[420px] lg:h-[450px]">
